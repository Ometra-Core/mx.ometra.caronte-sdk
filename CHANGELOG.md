...
- Updated Bee Hive to its Laravel 12/13-compatible 3.1 development line.

## [8.7.0] - 2026-08-24

### Added

- Added the generic `validate-group-session-config` Composer binary for validating shared application-session
  configuration from versioned JSON without exposing sensitive values.
...
- Keep the React `useId` hook order stable when the tenant selector is hidden.
